void draw_db_def(TString varname = "test") {
  
  cout << "Set Title: " << endl ;
  // TString varname = "test";
  cout << varname << endl ;
  ...
  // DB: editing y range
  graphScan->GetYaxis()->SetRangeUser(0.0,20.0);
  graphScan->GetXaxis()->SetRangeUser(-2.0,2.0);
  graphScan->SetTitle(varname);
  cc->Print("../../ll_plots/"+varname + ".png");


  // DB: estimate of the 1sigma and 2sigma intervals with a pol8 fit
  // working only for curves with 1 minimum
  // VERY INFORMAL method.. just useful to read values fast   

  TF1 *ffit = new TF1("ffit","pol8") ;
  TCanvas *cDani = new TCanvas();
  graphScan->Draw();
  graphScan->Fit("ffit","","",-1.,1.);

  if(ffit->Eval(-1.) < 4 || ffit->Eval(1.) < 4  ) {
    printf("\nWarning: ffit(1) = %f\n",ffit->Eval(1.)) ;
    graphScan->Fit("ffit");
  }
    
  double sigma1_l = ffit->GetX(1,-2.,0.) ; 
  double sigma1_r = ffit->GetX(1,0.,2.) ;
  double sigma2_l = ffit->GetX(3.84,-2.,0.) ;
  double sigma2_r = ffit->GetX(3.84,0.,2.) ;
  
  // print results... 
  cout << endl << "1 sigma interval:"  << endl ;
  printf("(%1.2f,%1.2f)\n",sigma1_l,sigma1_r);
  
  cout << endl << "2 sigma interval:"  << endl ;  
  printf("(%1.2f,%1.2f)\n",sigma2_l,sigma2_r);

  int ndf = ffit->GetNDF();
  double chi_square = ffit->GetChisquare();
  printf("\nX^2/ndf = %f\n", chi_square/ndf);

  ofstream outfile;
  outfile.open("../../confidence_intervals.csv",std::ios_base::app);

  outfile << varname << "," << sigma1_l << "," << sigma1_r << "," << sigma1_r-sigma1_l << "," << sigma2_l << "," << sigma2_r << "," << sigma2_r-sigma2_l << endl ; 
  outfile.close();
