...
# ---------------------------------------- ZZ 
samples['ZZ_LO'] = {  'name'  :   nanoGetSampleFiles(MCDir,'ggZZ4m')
                             ...
                             + nanoGetSampleFiles(MCDir,'ggZZ2e2n'),
                    'weight' :  XSWeight+'*samesign_requirement*'+SFweight+'*((nLepton==2)*PromptGenLepMatch2l + (nLepton==3)*PromptGenLepMatch3l + (nLepton>3)*PromptGenLepMatch4l)*'+METFilter_MC,
                    'FilesPerJob' : 3,
                 }

ggZZbaseW   = getBaseWnAOD(MCDir,'Autumn18_102X_nAODv7_Full2018v7',['ggZZ4m',        'ggZZ4m_ext1'])

addSampleWeight(samples,'ZZ_LO','ggZZ2e2t',      "1.68") ## The NLO/LO k-factor, cited from https://arxiv.org/abs/1509.06734v1
addSampleWeight(samples,'ZZ_LO','ggZZ2m2t',      "1.68")
addSampleWeight(samples,'ZZ_LO','ggZZ2e2m',      "1.68")
addSampleWeight(samples,'ZZ_LO','ggZZ4m',        "1.68*"+ggZZbaseW+"/baseW")
addSampleWeight(samples,'ZZ_LO','ggZZ4m_ext1',   "1.68*"+ggZZbaseW+"/baseW")
addSampleWeight(samples,'ZZ_LO','ggZZ4t',        "1.68")
