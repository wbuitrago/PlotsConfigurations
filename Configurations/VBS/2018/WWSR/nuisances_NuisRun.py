...
nuisances['lumi_Uncorrelated'] = {
    ...
    'type': 'lnN',
    'samples': dict((skey, '1.015') for skey in mc if skey not in ['WZ_QCD','ZZ','tZq']) # data driven: WZ QCD, ZZ, tVx
}
# Beam current calibration,0.2,0.3,0.2
nuisances['lumi_Beamcalibration'] = {
    'name': 'lumi_beamcalib_2018',
    'type': 'lnN',
    'samples': dict((skey, '1.002') for skey in mc if skey not in ['WZ_QCD','ZZ','tZq']) # data driven: WZ QCD, ZZ, tVx
}
...
nuisances['lumi_ghost_sat'] = {
    ...
    'type': 'lnN',
    'samples': dict((skey, '1.001') for skey in mc if skey not in ['WZ_QCD','ZZ','tZq']) # data driven: WZ QCD, ZZ, tVx
}
...
nuisances['lumi_Uncorrelated'] = {
    'name': 'lumi_13TeV_2018',
    'type': 'lnN',
    'samples': dict((skey, '1.02') for skey in mc if skey not in ['WZ_QCD','ZZ','tZq']) # data driven: WZ QCD, ZZ, tVx
}
...
nuisances['electronpt'] = {
    ...
    'folderUp': MCDir+'__ElepTup_suffix',
...
nuisances['muonpt'] = {
    ...
    'mapDown': 'MupTdo',
    'samples': dict((skey, ['1', '1']) for skey in mc),
    'folderUp': MCDir+'__MupTup_suffix',
    'folderDown': MCDir+'__MupTdo_suffix',
    'AsLnN': '1'
}
...
for js in jes_systs:
  if 'Absolute' in js: 
    folderup = MCDir+'__JESAbsoluteup_suffix'
    folderdo = MCDir+'__JESAbsolutedo_suffix'
  elif 'BBEC1' in js:
    folderup = MCDir+'__JESBBEC1up_suffix'
    folderdo = MCDir+'__JESBBEC1do_suffix'
  elif 'EC2' in js:
    folderup = MCDir+'__JESEC2up_suffix'
    folderdo = MCDir+'__JESEC2do_suffix'
  elif 'HF' in js:
    folderup = MCDir+'__JESHFup_suffix'
    folderdo = MCDir+'__JESHFdo_suffix'
  elif 'Relative' in js:
    folderup = MCDir+'__JESRelativeup_suffix'
    folderdo = MCDir+'__JESRelativedo_suffix'
  elif 'FlavorQCD' in js:
    folderup = MCDir+'__JESFlavorQCDup_suffix'
    folderdo = MCDir+'__JESFlavorQCDdo_suffix'
...
nuisances['pdf_weight'] = { # --> Now save also the normalization one for the signal
    ...
    'type'  : 'shape',
    'samples' :  { s: [' Alt$(LHEPdfWeight['+str(i)+'], 1.)' for i in range(0,103)] for s in mc if s not in ['DPS']},
    'AsLnN':  '1'
}
...
nuisances['QCD_scale_QCD_WW_accept'] = {
