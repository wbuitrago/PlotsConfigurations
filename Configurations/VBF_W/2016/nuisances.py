...
mc =["WLNuJJ", "Wjets_HT", "top", "VV", "VBS", "ggWW", "Higgs", "DY_M-50", "DY_else", "VBF-Z", "Vg", "VVV"]
...
# # ##################################
# # ######## Fatjet uncertainties (THIS GIVES PROBLEM IN INCLUDING THE ALIASES FOR VBS_CATEGORY!!!)
...
# FileJson = conf_folder + "utils/nuisance_incl_norm_factors_2018.json"
# with open(FileJson) as jsonFile:
#     nuis_factors = json.load(jsonFile)
#     jsonFile.close()


# #nuis_factors = json.load(open("/afs/cern.ch/user/a/abulla/CMSSW_10_6_4/src/PlotsConfigurations/Configurations/VBF_W/2018/sys/nuisance_incl_norm_factors_2018.json"))

# for sample in mc :
#     if sample in ["ggWW","VV", "VBS", "Wjets_HT"]: continue
...
#             'samples': { k:["QCDscale_normalized[0]", "QCDscale_normalized[8]"] for k in ["VBS"] }
#         }
...
#             'kind'  : 'weight',
#             'type'  : 'shape',
#             'samples': { k:["QCDscale_normalized[0]", "QCDscale_normalized[8]"] for k in ["VV"] }
#         }
...
#             'kind'  : 'weight',
#             'type'  : 'shape',
#             'samples': { k:["LHEScaleWeight[0]", "LHEScaleWeight[8]"] for k in ["VV"]}
#         }
...
#             'kind'  : 'weight',
#             'type'  : 'shape',
#             'samples'  :  { sample: ["LHEScaleWeight[0]", "LHEScaleWeight[8]"] for sample in ["Wjets_HT"]}
#         }
...
#                          'PSWeight[0] * {}'.format(nuis_factors[s]["PS_ISR"][1]) ] for s in mc if s not in ["Wjets_HT","Higgs"] }
#             }
...
#                          'PSWeight[1] * {}'.format(nuis_factors[s]["PS_FSR"][1]) ] for s in mc if s not in ["Wjets_HT","Higgs"]}
#             }
...
#                          '(puWeightDown/puWeight) * {}'.format(nuis_factors[s]["CMS_PU_2018"][1])] for s in mc if s not in ["Wjets_HT","Higgs"]},
#                 'AsLnN'      : '1',
# }
...
#                         '(puWeightDown/puWeight)'] for s in ["Wjets_HT"]},
#             'AsLnN'      : '1',
# }
...
#     'type'  : 'shape',
#     'samples' :  { s: [' Alt$(LHEPdfWeight['+str(i)+'], 1.)' for i in range(0,103)] for s in mc if s not in ["VBS", "top","Wjets_HT","VBS"]},
#     'AsLnN':  '1'
# }
...
#     'samples': { k : [ 'Alt$(PDFweight_normalized['+str(i)+'], 1.)' for i in range(0,103) ] for k in ["VV"]}
# }
...
#                 'samples'  : {'Wjets_HT_hardJets' : '1.00'},
#                 'type'  : 'rateParam',
