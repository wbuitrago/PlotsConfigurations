...
public:
  ...
  TTreeFunction* clone() const override { return new Wzinc(); }
...
Wzinc::evaluate(unsigned)
{
  ...
  if (!(Lepton_pt->At(iPromptL[zlepton1_idx])>25 && Lepton_pt->At(iPromptL[zlepton2_idx])>10 && Lepton_pt->At(iPromptL[wlepton_idx])>25)){
    return 0;
  }
  ...
  if (pl0l1.M()<=4 && pl0l2.M()<=4 && pl1l2.M()<=4)
    return 0.;
