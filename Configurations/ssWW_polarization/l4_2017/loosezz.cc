...
class Loosezz : public multidraw::TTreeFunction {
public:
  Loosezz();

  char const* getName() const override { return "Loosezz"; }
  TTreeFunction* clone() const override { return new Loosezz(); }
...
Loosezz::evaluate(unsigned)
{
  ...
  //  return 0.;
  //}
  if(!(Lepton_pt->At(0)>25 && Lepton_pt->At(1)>20 && Lepton_pt->At(2)>10 && Lepton_pt->At(3)>10)){
    return 0.;
  }
  if(!(abs(z0Mass_min-91.1876)<30 && abs(z1Mass_min-91.1876)<30)){
    return 0.;
  }

  //double Met{*MET_pt->Get()};
  //if (Met<=30)
  //  return 0.;
...
void
Loosezz::bindTree_(multidraw::FunctionLibrary& _library)
{
